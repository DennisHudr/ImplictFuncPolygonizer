...
typedef struct process {	   /* parameters, function, storage */
    ...
    triangleProc     triproc;		   /* triangle output function */
...
void makecubetable();
void converge (POINT* p1, POINT* p2, double v,ImplicitFunction function, POINT* p);
...
int main ()
    ...
    if ((err = polygonize(blob, .05, 20, 0.,0.,0., triangle, TET)) != NULL) {
	fprintf(stdout, "%s\n", err);
	exit(1);
	}
